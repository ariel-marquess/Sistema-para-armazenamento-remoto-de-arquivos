...
class menuFile(ctk.CTkFrame):
    def __init__(self, master, event, path, forced_widget=None):
        ...
        if sort == "arquivo":
            ...
            download = ctk.CTkButton(self, text="Baixar", anchor="w", fg_color="transparent",
                        command=lambda s=self, download=ctrl.download, p=path: util.execute(s, download, [p]))  # Tenho que estabelecer qual será o diretório do arquivo
            ...
            delete = ctk.CTkButton(self, text="Apagar", anchor="w", fg_color="transparent",
                        command=lambda s=self, delete=ctrl.delete, t=target, p=path: util.execute(s, delete, [t, p]))
